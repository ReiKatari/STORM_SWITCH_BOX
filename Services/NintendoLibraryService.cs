...
namespace StormSwitchBox.Services
{
    ...
    /// <summary>
    /// Сервис базы данных и каталога библиотеки игр всех 19 поколений Nintendo (включая Switch 2),
    /// с полной поддержкой форматирования чисел с разделителями тысяч, стандартизацией дат (dd.MM.yyyy),
    /// чистых версий без префикса v и оригинальными обложками.
    /// </summary>
    public class NintendoLibraryService
    {
        ...
        private readonly List<NintendoGameEntry> _database = new();
        private readonly HashSet<string> _loadedSwitchIds = new(StringComparer.OrdinalIgnoreCase);
        ...
        /// <summary>
        /// Форматирует любую дату или год в строгий формат dd.MM.yyyy
        /// </summary>
        public static string FormatDate(string? dateStr)
        {
            ...
            // Попытка разбора стандартных форматов yyyy-MM-dd, yyyyMMdd, yyyy/MM/dd
            ...
            // Если указан только год (например "1985")
        ...
        public void EnsureSwitchGamesLoaded(TitleDbService? titleDb)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                if (_loadedSwitchIds.Contains(entry.Id)) continue;
                ...
                _database.Add(new NintendoGameEntry
                {
                    Id = entry.Id,
                    Title = entry.Name,
        ...
        private void InitializeDatabase()
        {
            ...
            // =========================================================================
            // 3. NES / Famicom (1983) - Полная коллекция культовых хитов и версий
            // =========================================================================
            ...
            // =========================================================================
            // 16. Nintendo 3DS (2011) - Полная коллекция
            // =========================================================================
        ...
        public List<NintendoGameEntry> QueryGames(
            ...
            string? sortBy = "Title")
        {
            IEnumerable<NintendoGameEntry> result = _database;

            if (!string.IsNullOrEmpty(systemFullName) && systemFullName != "Все системы")
            {
                result = result.Where(g => g.System.Equals(systemFullName, StringComparison.OrdinalIgnoreCase) ||
                                           g.SystemShort.Equals(systemFullName, StringComparison.OrdinalIgnoreCase));
            }
        ...
        {
            var source = (!string.IsNullOrEmpty(systemFullName) && systemFullName != "Все системы")
                ? _database.Where(g => g.System.Equals(systemFullName, StringComparison.OrdinalIgnoreCase) ||
                                       g.SystemShort.Equals(systemFullName, StringComparison.OrdinalIgnoreCase))
                : _database;
            ...
            return list.OrderBy(x => x == "Все жанры" ? "" : x).ToList();
        }
        ...
        public List<string> GetDistinctDevelopers(string? systemFullName = null)
        {
            ...
            var list = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            list.Add("Все разработчики");
            foreach (var g in source)
            {
                if (!string.IsNullOrEmpty(g.Developer)) list.Add(g.Developer);
            }
            return list.OrderBy(x => x == "Все разработчики" ? "" : x).ToList();
        }
        ...
        public List<string> GetDistinctPublishers(string? systemFullName = null)
        {
            ...
            var list = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            list.Add("Все издатели");
            foreach (var g in source)
            {
                if (!string.IsNullOrEmpty(g.Publisher)) list.Add(g.Publisher);
            }
            return list.OrderBy(x => x == "Все издатели" ? "" : x).ToList();
        }
