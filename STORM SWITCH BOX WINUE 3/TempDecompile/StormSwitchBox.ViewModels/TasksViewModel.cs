...
public class TasksViewModel : ObservableObject
{
	...
	public string SelectedFormat
	{
		...
		[MemberNotNull("_selectedFormat")]
		set
		{
			if (!EqualityComparer<string>.Default.Equals(_selectedFormat, value))
			{
				OnPropertyChanging(__KnownINotifyPropertyChangingArgs.SelectedFormat);
				_selectedFormat = value;
				OnPropertyChanged(__KnownINotifyPropertyChangedArgs.SelectedFormat);
			}
		}
	}
	...
	public int SelectedFormatIndex
	{
		...
		set
		{
			if (!EqualityComparer<int>.Default.Equals(_selectedFormatIndex, value))
			{
				OnPropertyChanging(__KnownINotifyPropertyChangingArgs.SelectedFormatIndex);
				_selectedFormatIndex = value;
				OnPropertyChanged(__KnownINotifyPropertyChangedArgs.SelectedFormatIndex);
			}
		}
	}
			...
			else
			{
				...
				else
				{
					if (isDirectory)
					{
						...
						{
							List<(string Path, string? TitleId, string Type, string TopFolder, byte[]? IconBytes)> filesMeta = new List<(string, string, string, string, byte[])>();
							List<Task<(string Path, string TitleId, string Type, string TopFolder, byte[] IconBytes)>> metaTasks = gameFiles.Select(async delegate(string f)
							{
							...
							Dictionary<string, string> topFolderTids = (from m in filesMeta
								where m.TitleId != null && m.TitleId.Length == 16
								group m by m.TopFolder).ToDictionary((IGrouping<string, (string Path, string TitleId, string Type, string TopFolder, byte[] IconBytes)> g) => g.Key, (IGrouping<string, (string Path, string TitleId, string Type, string TopFolder, byte[] IconBytes)> g) => (from m in g
								group m by m.TitleId.Substring(0, 12) into x
								orderby x.Count() descending
								select x).First().Key);
							IEnumerable<IGrouping<string, (string Path, string TitleId, string Type, string TopFolder, byte[] IconBytes)>> groups = filesMeta.GroupBy<(string, string, string, string, byte[]), string>(delegate((string Path, string TitleId, string Type, string TopFolder, byte[] IconBytes) m)
							{
								string text = "UNKNOWN";
								string value;
								...
								else if (topFolderTids.TryGetValue(m.TopFolder, out value))
								{
									text = value;
								}
								return text + "_" + m.TopFolder;
							});
							foreach (IGrouping<string, (string, string, string, string, byte[])> group in groups)
							{
								List<(string Path, string TitleId, string Type, string TopFolder, byte[] IconBytes)> filesInGroup = group.ToList();
	...
	private void AddOrUpdateTask(List<string> files, string groupId, string basePath, byte[]? iconBytes = null)
	{
		...
		App.MainDispatcher?.TryEnqueue(async delegate
		{
			...
			if ((_currentPageType == "Update" || _currentPageType == "Multi") && groupId != null && groupId.Length == 12)
			{
				existingTask = Tasks.FirstOrDefault((ProcessingTask t) => t.GroupId == groupId && t.Operation == _currentPageType && t.Status == "Ожидание");
			}
			if (existingTask != null)
			{
				...
				if (added > 0)
				{
					...
					string[] dirs = existingTask.InputFiles.Select((string path) => Path.GetDirectoryName(path)).Distinct().ToArray();
			...
			else
			{
				...
				if (groupId != null && groupId.Length >= 12 && Regex.IsMatch(groupId.Substring(0, 12), "^[0-9a-fA-F]{12}$"))
				{
					...
					if (App.TitleDb.TryGetTitleInfo(tid16, out TitleDbEntry entry) && entry != null)
					{
		...
		Task.Run(async delegate
		{
			try
			{
				while (_isProcessingQueue)
				{
					...
					if (nextTask != null)
					{
						ExecuteTaskSafelyAsync(nextTask);
					}
					else
					{
	...
	private async Task ExecuteTaskSafelyAsync(ProcessingTask task)
	{
		try
		{
			...
			App.MainDispatcher?.TryEnqueue(delegate
			{
				if (task.Status == "Подготовка..." || task.Status == "Сжатие..." || task.Status == "Распаковка..." || task.Status == "Сборка...")
				{
					task.Status = "Успешно";
