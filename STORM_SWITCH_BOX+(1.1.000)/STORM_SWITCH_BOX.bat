...
function YEStart {
    ...
    $script:title = 'STORM SWITCH BOX (1.1.000)'
    $yanu_rec = '0.10.1'
...
function Get-TitleIdFromNsz {
    ...
    try {
        ...
        # Приоритет: Base (000) → Update (800) → первый
    ...
    try {
        ...
        switch ($taskData.TaskType) {
            ...
            'DirectStitch' {
                ...
                $utf8NoBom = New-Object System.Text.UTF8Encoding $false
                [System.IO.File]::WriteAllLines($nscbListPath, $localFilesForStitching, $utf8NoBom)
                $isolatedNscb = Setup-IsolatedToolWithKeys -toolName 'nscb' -tempDir $tempDir -toolPaths $toolPaths
            ...
            'BuildMulti' {
                ...
                if ($hasMods) {
                    ...
                    foreach ($ncaFile in (Get-ChildItem -LiteralPath $pdata -Filter *.nca)) {
                        $hactoolOutput = & $toolPaths.hactoolnet -k "$($toolPaths.key)" "$($ncaFile.FullName)" 2>&1 | Out-String
                    ...
                    $titleIdOutput = & $toolPaths.hactoolnet -k "$($toolPaths.key)" "$($controlNcaFile.FullName)" 2>&1 | Out-String
                ...
                if ($true) {
                    ...
                    if ($filesForStitching.Count -gt 1 -or (-not [string]::IsNullOrWhiteSpace($taskData.InternalName))) { 
                        ...
                        $utf8NoBom = New-Object System.Text.UTF8Encoding $false
                        [System.IO.File]::WriteAllLines($nscbListPath, $filesForStitching, $utf8NoBom);
                        $isolatedNscb = Setup-IsolatedToolWithKeys -toolName 'nscb' -tempDir $tempDir -toolPaths $toolPaths;
                        $squirrelArgsArray = @('-b', '65536', '-pv', 'false', '-kp', $kpVal, '--RSVcap', '268435656', '-fat', 'exfat', '-fx', 'files', '-ND', 'true', '-t', $nscbTargetType, '-o', "`"$packDir`"", '-tfile', "`"$nscbListPath`"", '-roma', 'TRUE', '-dmul', '"calculate"');
        ...
        if ($taskData.TaskType -ne 'Unpack' -and $null -ne $finalFileInTemp -and (Test-Path -LiteralPath $finalFileInTemp)) {
            ...
            if ($finalFileSizeBytes -gt 0 -and $finalFileSizeBytes -le 1024) {
        ...
        foreach ($procId in $procIds) {
            ...
            if ($process) {
                ...
                if ($crashLog) {
                    ...
                    if ($finalResult.Status -eq 'Готово') { Write-Log "Задача '$($taskRow.Cells['Задача'].Value)' для '$($taskRow.Cells['Обработка'].Value)' успешно завершена." -TaskID $taskRow.Tag.TaskID -Type 'SUCCESS'; Send-ToastNotification -Title '✅ Задача завершена' -Message "'$($taskRow.Cells['Обработка'].Value)' успешно обработано." -Type 'Success' } else { Write-Log "Задача '$($taskRow.Cells['Задача'].Value)' для '$($taskRow.Cells['Обработка'].Value)' завершилась со статусом: $($finalResult.Status)." -TaskID $taskRow.Tag.TaskID -Type 'ERROR'; Send-ToastNotification -Title '❌ Ошибка задачи' -Message "'$($taskRow.Cells['Обработка'].Value)' завершилась с ошибкой." -Type 'Error' }
                } else {
