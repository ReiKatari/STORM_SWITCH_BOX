...
В версии **3.9.7** добавлены следующие ключевые улучшения:
*   **🔄 Автоматическая миграция настроек**: Добавлена встроенная система синхронизации сохранённых параметров пользователя при обновлении приложения. Значения *Пересборка в Мульти-контенте* и *Комплексные имена по папкам* гарантированно становятся включёнными, а *Удалять неиспользуемые языки* — выключенным.
*   **🧹 Фильтрация технических логов**: Из лога задачи в Мульти-контенте полностью удалены ложные предупреждения `[NSC_Builder Error]` для прогресс-бара tqdm и спам тысяч мелких файлов распаковки `hacpack`.
*   **🎯 Точное позиционирование окна при старте**: Добавлен интеллектуальный расчёт координат первого запуска по центру экрана (`Primary WorkArea`).
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.7_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
...
1. Нажмите правой кнопкой мыши на скачанный файл архива `STORM_SWITCH_BOX_3.9.7_win-x64.zip`.
