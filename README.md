...
В версии **3.9.10** добавлены следующие ключевые улучшения:
*   **🔥 Полная ликвидация ложного сбоя "Ошибка предварительной сборки (HardPatch)"**: Удалён устаревший дублирующий блок предварительной сборки из `TasksViewModel.cs`. Поток мульти-контента теперь на 100% управляется единым изолированным сервисом `MultiContentService`, что гарантирует идеальное сшивание комплектов Базовой игры + Всех DLC без сбоев.
*   **🚀 Оптимизация производительности при сборке Мульти-контента**: Убраны повторные проверки файлов перед отправкой задачи в обработку.
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.10_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
...
1. Нажмите правой кнопкой мыши на скачанный файл архива `STORM_SWITCH_BOX_3.9.10_win-x64.zip`.
