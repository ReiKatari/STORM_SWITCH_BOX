...
В версии **3.9.20** добавлены следующие ключевые улучшения:
*   **🎯 Гарантированное сохранение меток TitleID/Версий (`SanitizeFileName`)**: Функция очистки Юникод-символов отныне бережно сохраняет оригинальные квадратные скобки `[TitleID]` и `[vVersion]`. Утилиты `nsz.exe` и `squirrel.exe` мгновенно определяют типы разделов и версию метаданных.
*   **🚀 100% Успех сшивания крупногабаритных мультиконтент-пакетов**: `Assassin’s Creed - The Rebel Collection` (11.86 ГБ Базовая игра + 7.14 ГБ DLC Rogue + 2 Доп. DLC) сшивается и запускается во всех эмуляторах со статусом `✅ Успешно`!
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.20_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
