...
В версии **3.9.13** внедрены ключевые этапы глубокой архитектурной оптимизации:
*   **⚡ Буферизация логов (`LogBatcher`)**: Консольный вывод консольных утилит накапливается в изолированном сервисе и передаётся в UI-поток пакетами. Полностью исключены микрофризы WinUI 3 при массовой распаковке.
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.13_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
