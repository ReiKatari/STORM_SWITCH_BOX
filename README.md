...
В версии **3.9.14** добавлены следующие ключевые улучшения:
*   **🔥 Полная ликвидация `OSError [Errno 22]` и байта UTF-8 BOM (`\ufeff`)**: Файл списка `mlist.txt` теперь записывается в чистом формате `UTF8` без маркера последовательности байтов (`BOM`). Базовая игра `[v0]` больше **никогда не пропускается утилитой NSC_Builder**, а размер собранного пакета полностью соответствует комплекту! Сшитая игра идеально запускается во всех эмуляторах!
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.14_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
