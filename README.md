...
В версии **3.9.2** добавлены следующие ключевые улучшения:
*   **🔑 Исключение предзашитых ключей и сквозная синхронизация**: Из дистрибутива полностью удалены тестовые файлы ключей. Если пользователь еще не указал свой `prod.keys` / `keys.txt` в Параметрах, приложение выдает информативное сообщение. Добавленный пользователем файл автоматически подхватывается во всех модулях (`NSC_Builder`, `yanu-cli`, `nsz`, `LibHac`).
*   **⚙️ Исправление рабочей директории и перехват логов NSC_Builder**: Задана строгая рабочая директория `WorkingDirectory` для процесса `squirrel.exe`, что полностью устранило ошибку `exit code 1` при сборке мультиконтента. Реализован трансляционный вывод консоли `NSC_Builder` в журнал задач в реальном времени.

Предыдущие изменения (версия **3.9.1**):
*   **🛠️ Бесшовное закрытие приложения при обновлении/установке**: Автоматическая выгрузка работающих процессов перед заменой файлов.
*   **🔄 Синхронизированный скрипт сборки инсталлятора**: Принудительная публикация перед упаковкой `Setup.exe`.
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.2_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
...
1. Нажмите правой кнопкой мыши на скачанный файл архива `STORM_SWITCH_BOX_3.9.2_win-x64.zip`.
