...
В версии **3.8.3** добавлены следующие улучшения:
*   **📥 Исправлено тихое удаление**: В Pascal-скрипт инсталлятора интегрированы проверки 32-битного реестра (`HKLM32`) и запуск через `ShellExec` для автоматического обхода UAC при удалении старых версий (даже установленных от имени администратора).
*   **📌 Сворачивание в системный трей**: При нажатии кнопки закрытия (крестик) приложение сворачивается в трей (с возможностью развертывания или выхода через контекстное меню). При обычном свертывании приложение скрывается в панель задач.
*   **📁 Корректное имя ярлыка**: Создаваемый ярлык теперь называется `STORM_SWITCH_BOX` в соответствии с названием папки установки.
*   **📦 Полная автономность**: Сборка поставляется как Self-Contained со встроенным .NET 8 Runtime, решая проблему требования скачивания .NET при старте.
...
> [!NOTE]
> Выпускаемый в релизах архив `StormSwitchBox_v3.8.3_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
...
1. Нажмите правой кнопкой мыши на скачанный файл архива `StormSwitchBox_v3.8.3_win-x64.zip`.
