...
1. Подписанный инсталлятор: **`STORM_SWITCH_BOX_4.7.0_Setup.exe`** (с отметкой времени RFC 3161)
2. Бандл установки (для Smart App Control): **`STORM_SWITCH_BOX_4.7.0_Setup_Bundle.zip`**
3. Портативный архив: **`STORM_SWITCH_BOX_4.7.0_win-x64.zip`**

---

## 🛡️ Цифровая подпись и Smart App Control (Интеллектуальное управление приложениями)

Все исполняемые файлы, системные библиотеки и инсталлятор подписаны официальным сертификатом **Authenticode SHA-256** издателя **STORM TEAM** с подтверждённой доверенной отметкой времени RFC 3161 (DigiCert TSA).

> [!TIP]
> **Если Windows 11 Smart App Control (Интеллектуальное управление приложениями) блокирует скачанный `.exe`:**
> 1. Скачайте **`STORM_SWITCH_BOX_4.7.0_Setup_Bundle.zip`**, распакуйте его и запустите файл **`Запустить_Установку.cmd`** (он автоматически снимет метку Zone.Identifier и зарегистрирует сертификат STORM TEAM в доверенных корневых центрах сертификации).
> 2. Либо запустите **`Установка_Сертификата.bat`** перед запуском установщика.
