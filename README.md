...
В версии **3.9.12** добавлены следующие ключевые улучшения:
*   **🎮 Устранение ошибки "The NSP file is missing a Program-type NCA"**: Ликвидирован фальшивый маппинг `CreateSymbolicLink`. Файлы передаются в `NSC_Builder` по оригинальным прямым путям, гарантируя 100% включение базовых `Program-type NCA` разделов. Сшитые игры снова идеально открываются в эмуляторах Yuzu, Ryujinx и Eden!
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.12_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
