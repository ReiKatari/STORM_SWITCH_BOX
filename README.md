...
В версии **3.9.19** добавлены следующие ключевые улучшения:
*   **🔥 Полное устранение `UnicodeEncodeError: 'charmap' codec can't encode...`**: Внедрена функция `PrepareSafeFileForTemp`, автоматически нормализующая Юникод-апострофы (`’` -> `'`) и тире в путях входных файлов.
*   **⚡ Флаг `--quick-verify` для `nsz.exe`**: Убраны ложные вылеты `[CORRUPTED]` при распаковке файлов `.nsz` / `.xcz` со сторонними хэшами. `Assassin’s Creed - The Rebel Collection` и любые игры с Юникод-символами в именах сшиваются 100% идеально!
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.19_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
