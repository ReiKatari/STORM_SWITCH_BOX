...
В версии **3.9.23** добавлены следующие ключевые улучшения:
*   **🔥 Полная ликвидация `UnicodeEncodeError` в `squirrel.exe` (NSC_Builder)**: Имена игр из TitleDB (например, `Assassin's Creed` с Unicode-апострофом `'` U+2019) теперь полностью санитизируются через `SanitizeFileName` перед формированием выходного пути, предотвращая крах `cp1251.py encode` в PyInstaller-замороженных Python-утилитах.
*   **🛡️ Принудительный UTF-8 для всех PyInstaller-процессов**: Добавлен `RedirectStandardInput` + `PYTHONCOERCECLOCALE=1`, чтобы PyInstaller-процессы (`squirrel.exe`, `nsz.exe`) не наследовали кодовую страницу консоли Windows (cp1251) и гарантированно использовали UTF-8 для stdout/stderr.
*   **🔎 Улучшенный поиск декомпрессированных файлов после `nsz.exe`**: Если `nsz.exe` создаёт выходной файл с именем, отличающимся от ожидаемого (например, из внутренних метаданных NCA), движок теперь сканирует всю выходную директорию для обнаружения результата, устраняя ложный откат на Zero-Disk-IO и ошибку `Could not find file`.

Предыдущие изменения (версия **3.9.22**):
*   **🔥 Прямой запуск `squirrel.exe` без подмененой кодировки `cmd.exe`**: Убран сбрасывающий оболочку вызов `cmd.exe /c chcp 65001`. Переменные `PYTHONUTF8=1` и `PYTHONIOENCODING=utf-8` отныне напрямую попадают в PyInstaller.
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.23_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
