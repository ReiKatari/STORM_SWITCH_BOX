...
В версии **3.9.0** добавлены следующие ключевые улучшении:
*   **🔑 Гарантированное обнаружение ключей в yanu-cli**: Добавлен прямой проброс параметра `--keyfile` (`-k`) во все вызовы `yanu-cli.exe` (update, pack, unpack). Это полностью устранило ошибку `Failed to find keyfile` при применении HardPatch и сборке пакетов на компьютерах любых пользователей.
*   **🎮 Исправлена сборка мультиконтента для эмуляторов**: Устранена ошибка «Ваш ROM зашифрован» в эмуляторе Eden и Yuzu/Ryujinx. Сборка мультиконтента переведена на `NSC_Builder (squirrel)` с автоматической конвертацией тикетных NCA в Standard Crypto и сведением контейнерных метаданных CNMT.
*   **🎨 Адаптивный дизайн журнала событий**: Исправлен визуальный баг «белая и черный фон снизу» — панель деталей и логов переведена на системные кисти WinUI 3 (`ThemeResource`), идеально сочетаясь со светлой и тёмной темами Windows.
*   **📌 Восстановление окна из трея**: Исправлена активация приложения при повторном запуске с ярлыка — окно автоматически разворачивается из системного трея и выводится на передний план.
*   **📦 Полная автономность (Self-Contained)**: В проект включена гарантированная публикация встроенного .NET 8 Runtime, благодаря чему программа не требует отдельной установки .NET Core у пользователей.
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.0_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
...
1. Нажмите правой кнопкой мыши на скачанный файл архива `STORM_SWITCH_BOX_3.9.0_win-x64.zip`.
