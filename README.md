...
В версии **3.9.17** добавлены следующие ключевые улучшения:
*   **🛡️ Доверенный манифест безопасности `app.manifest`**: Добавлена прозрачная декларация привилегий `requestedExecutionLevel level="asInvoker" uiAccess="false"` и совместимости `<supportedOS>` для Windows 10/11 (`{8e0f7a12-bfb3-4fe8-b9a5-48fd50a15a9a}`). Это предотвращает классификацию WinUI 3 приложения фильтрами Smart App Control и Защитником Windows как нераспознанного UAC-процесса.
*   **📂 Включение поддержки длинных путей (`longPathAware`)**: В манифест добавлена функция `longPathAware` для работы с глубоко вложенными путями образ-файлов.
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.17_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
