...
В версии **3.8.2** добавлено важное исправление для применения патчей с romfs-модами (устранена ошибка отображения базовой версии 1.0.0 вместо версии патча), а также подавлены лишние предупреждения NETSDK1057 при сборке.
...
> [!NOTE]
> Выпускаемый в релизах архив `StormSwitchBox_v3.8.2_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
...
1. Нажмите правой кнопкой мыши на скачанный файл архива `StormSwitchBox_v3.8.2_win-x64.zip`.
