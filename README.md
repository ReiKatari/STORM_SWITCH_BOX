...
В версии **3.9.16** добавлены следующие ключевые улучшения:
*   **🛡️ Автоматическая разблокировка Windows Smart App Control (SAC)**: В инсталлятор `setup.iss` и стартовую процедуру приложения внедрен асинхронный вызов `Unblock-File`, снимающий интернет-метку `Zone.Identifier` с инсталлятора и разворачиваемых бинарников.
*   **📋 Разблокировка в 1 клик**: В случае срабатывания фильтра Smart App Control добавлено руководство по снятию интернет-метки из свойств архива/инсталлятора.
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.16_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
