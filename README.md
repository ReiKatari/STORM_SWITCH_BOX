...
В версии **3.9.6** добавлены следующие ключевые улучшения:
...
*   **⚙️ Новые параметры по умолчанию**: *Пересборка файлов в Мульти-контенте* и *Комплексное выходное имя файла по папкам* по умолчанию **ВКЛЮЧЕНЫ**. *Удалять неиспользуемые языки* по умолчанию **ВЫКЛЮЧЕНО**.
...
> [!NOTE]
> Выпускаемый в релизах архив `STORM_SWITCH_BOX_3.9.6_win-x64.zip` является полностью **автономным (Self-Contained)**. Он содержит встроенную среду выполнения .NET 8, поэтому дополнительно скачивать и устанавливать .NET Runtime не требуется.
...
1. Нажмите правой кнопкой мыши на скачанный файл архива `STORM_SWITCH_BOX_3.9.6_win-x64.zip`.
