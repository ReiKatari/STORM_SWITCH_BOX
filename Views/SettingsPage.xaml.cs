            ...
            try
            {
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.NotFound)
                {
                    var dialog = new ContentDialog
                    {
                        ...
                        Content = new TextBlock { Text = "У вас установлена актуальная версия STORM SWITCH BOX v3.8.8." },
                        CloseButtonText = "OK",
                        XamlRoot = this.XamlRoot
                    };
                ...
                var currentVer = new Version("3.8.8");
                if (Version.TryParse(cleanTag, out var latestVer) && latestVer > currentVer)
                {
                    var dialog = new ContentDialog
                    {
                        ...
                        Content = new StackPanel
                        {
                            ...
                            Children =
                            {
                                new TextBlock { Text = $"Доступна версия: v{cleanTag} (Текущая: v3.8.8)", FontSize = 16, FontWeight = Microsoft.UI.Text.FontWeights.SemiBold },
